...
| `symbol-23.svg` | **Posuru** — a second mascot (a bear). | **No — confirmed 2026-08-19.** This *vector* is not used anywhere in the PickAxe worksheet; reserved for a different maze type's content. Note this is not a contradiction of §5 / `pdf_export_spec.md` §5, where Posuru does close the **Primary** last page: that page is supplied raster artwork with the character already drawn into it, so nothing places this SVG. |
